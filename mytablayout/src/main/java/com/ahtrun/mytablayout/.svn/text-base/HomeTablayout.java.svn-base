...
public class HomeTablayout extends FrameLayout {
    ...
    public void updateTabView(TabLayout.Tab tab, boolean isSelect){
        //找到自定义视图的控件ID
        try {
            TextView tabView = (TextView)(((LinearLayout) ((LinearLayout) tabLayout.getChildAt(0)).getChildAt(tab.getPosition())).getChildAt(1));
            if(isSelect) {
                //选中后字体变大
                tabView.setTextSize(TypedValue.COMPLEX_UNIT_PX,sp2px(context, 30));
                tabView.setTextAppearance(context, R.style.TabLayoutTextStyle);
            }else{
                //恢复为默认字体大小
                tabView.setTextSize(TypedValue.COMPLEX_UNIT_PX,sp2px(context, 14));
                tabView.setTextAppearance(context, Typeface.NORMAL);
